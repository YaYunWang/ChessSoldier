...
public abstract class ConfigLoaderBase
{
    ...
    public XmlDocument GetDocument(string path, string bundleName)
    {
        TextAsset data = AssetBundleManager.LoadAsset<TextAsset>(bundleName, System.IO.Path.GetFileNameWithoutExtension(path));
    ...
    private bool ReadPlainXml<T>(string path, System.Action<T> rowHandler)
    {
        ...
        XmlDocument doc = GetDocument(path, XmlConfigBundle);
