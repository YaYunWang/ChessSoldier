...
Shader "Demo03/Particle/Transparent/Alpha Additive (Dissolve)" {
    Properties {
    ...
    SubShader {
	    Tags { "Queue"="Transparent+1" "IgnoreProjector"="True" "RenderType"="Transparent" "AlphaFix" = "TransparentAdditive" }
	    LOD 50
	    ...
	    Pass {
			...
			UNITY_INSTANCING_CBUFFER_START(ParticlePropBlockAdditiveDissolve)
		    ...
		    fixed4 frag(v2f i) : COLOR {
				...
				fixed clipAmount = dissolve - UNITY_ACCESS_INSTANCED_PROP(_DissolveValue) - 
					UNITY_ACCESS_INSTANCED_PROP(_DissolveStride);
				...
				fixed4 col = lerp(edgeColor, 2 * texColor * UNITY_ACCESS_INSTANCED_PROP(_Color) * i.color, lerpVal);
